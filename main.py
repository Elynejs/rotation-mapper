...
start = time.time()
t1 = time.time()
end = t1 + 60 #change this number to change the duration(in seconds) of the mapping
log = []
i = 0
...
while True:
    ...
    i = i + 1
    if end <= start: 
        with open(f'log_file_{t1}.csv', "w") as file:
            columns = ['Key', 'Time (s)', 'Type']
            writer = csv.DictWriter(file, fieldnames=columns)
            for j in range(0,i):
                writer.writerow(log[j])
            df = pandas.read_csv(f'log_file_{t1}.csv', index_col='Name')
            print(df)
        break
